    ...
    % % % ----------------------------------------------------------------------------------------------------
    numSimulations = 1e3;
    extNumDropouts = 0; %%% It specifies wether to use the exact formula (23)=>1 or (24)=>0 of INFOCOM paper
    numYears = 1;
    eps_case = 2;
    if eps_case == 1        
        epsilon_b = eps;
    ...
    [expValPER,variancePER,maxNumConsecutiveDropouts,valGammaEpsp] = ...
        getAnalyticLinkQualityMetrics(nu_Gamma,sigma_Gamma,ell_F,epsilon_p,epsilon_b,...
            Ts,symFreq0,N0,W,P0,P1,se0,sb0,se1,sb1,te0,te1,V0,V1,dc,d0,d1,extNumDropouts);
    ...
    [P_Analytic_packet_rate, PEP_Analytic] = getMarkovChannelForContol(...
        TPM_Analytic_symbol_rate,nu_Gamma,sigma_Gamma,ell_F,GammaThresholds,GammaSteadyStateProbability,Ts,symFreq0,epsilon_p);
    [eta_M_Analytic,var_M_Analytic,nStarD_AnalyticNew] = getMarkovChannelLinkQualityMetricsNew(...
       TPM_Analytic_packet_rate,PEP_Analytic,GammaSteadyStateProbability,epsilon_b,epsilon_p,lengthBernoulli);
    % % % ----------------------------------------------------------------------------------------------------
    ...
    % % % ----------------------------------------------------------------------------------------------------
    % % components of the plant's output vector
%     J_b = getControlCost(x_B,u_B,Q,R,T);
%     J_m = getControlCost(x_M,u_M,Q,R,T);
%     cd(folderData)
%     load('config.mat', 'nameCostsComparison')
%     cd(folderFigures)
    %%% plotCosts(T,Ts,fullscreen,nameCostsComparison,J_L,J_B,J_M,J_m)
    %%% plotCostsSBM(T,Ts,fullscreen,nameCostsComparison,J_L,J_B,J_M)
